 ...
 *
 * MNCS is the semantic authority: `mncs/arrange.mncs` plus
 * `mncs/arrange-corpus.json` define the contract,
 ...
 * toolchain (source-study + portable-WASM and research-bytecode execution
 * with checked expectations, plus live calls into the compiled WASM).
...
export function mncsLoopTakeAllowed(saved: number): boolean {
  return saved < MNCS_MAX_LOOP_TAKES;
}
...
export function mncsScratchAllowed(count: number): boolean {
  return count < MNCS_MAX_SCRATCHES;
}
...
export function mncsClipActive(localTick: number, lengthTicks: number): boolean {
  return Number.isInteger(localTick) && localTick >= 0 && localTick < lengthTicks;
}
...
export function mncsClipLengthTicks(lengthTicks: number): number {
  return lengthTicks < 1 ? 1 : lengthTicks;
}
